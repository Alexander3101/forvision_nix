#' MAE accuracy tables and plots
#'
#' Generates MAE accuracy tables and plots.
#'
#' @aliases calculateMAE
...
#' @param sort if TRUE, sorts accuracy results according to the average MAE
#' rank across all available horizons.
#' @param digits number of digits after the dot used to output MdAPE; if digits=NA,
#' MAEs and ranks are not rounded.
#' @return a list with the following variables:
#' \describe{
#'   \item{accuracy:}{dataframe with accuracy table for MAE}
#'   \item{rank:}{ dataframe with MAE ranks}
#'   \item{plot:}{ggplot2 object showing accuracy vs horizon for MAE}
#' }
#' @details Calculates MAEs for each horizon and each method contained in af.
...
#' @export
#'
calculateMAE <- function(af, sort = FALSE, digits = 3){
  # Error handling
  # For TSTS schema
  if (!is.data.frame(af)){
  ...
  for(j in as.vector(unique(af$horizon))){
    for(i in as.vector(unique(af$method_id))){
      ...
      df[i, j] <-  round(mean(abs(value - forecast), na.rm=TRUE), digits)
    }
  }
  ...
  df <- cbind(df, " average MAE" = averageMdAPE)
